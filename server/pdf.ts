...
function supplyTable(items: SupplyItem[], showTotal: boolean, totalCents: number) {
  const rows = items.map(({ item, product, image }) => {
    ...
    return `<tr class="${escapeHtml(item.type.toLowerCase())}"><td class="qty">${structural ? "" : escapeHtml(item.quantity)}</td><td class="photo">${image ? `<img src="${image}" alt="">` : ""}</td><td class="description"><strong>${escapeHtml(title)}</strong>${brandModel ? `<small>Marca e modello: ${escapeHtml(brandModel)}</small>` : ""}${description ? `<p>${multiline(description)}</p>` : ""}</td><td class="price">${structural ? "" : formatCurrency(item.totalCents)}</td></tr>`;
  }).join("");
  return `<div class="supplies-panel"><table class="supply-table"><thead><tr><th class="qty">Q.TÀ</th><th class="photo">FOTO</th><th>PRODOTTO, MARCA, MODELLO E DESCRIZIONE</th><th class="price">PREZZO TOTALE<br><small>IVA INCLUSA</small></th></tr></thead><tbody>${rows}</tbody></table>${showTotal ? `<div class="supply-total"><span>PREZZO TOTALE<br><small>IVA INCLUSA</small></span><b>${formatCurrency(totalCents)}</b></div>` : ""}</div>`;
}
...
export async function quoteHtml(quoteId: number) {
  const [quote, settings, conditions, privacy, coverBg, customerBg, suppliesBg, summaryBg] = await Promise.all([
    ...
    templateData("cover.png"), templateData("customer.png"), templateData("supplies.png"), templateData("summary.png"),
  ]);
  ...
  const supplyItems: SupplyItem[] = await Promise.all(quote.items.map(async (item) => {
    ...
    return { item, product, image: await imageData(product?.imagePath) };
  }));
  ...
  const cover = `<section class="page template cover"><img class="page-bg" src="${coverBg}" alt=""><div class="cover-title"><b>PREVENTIVO</b><strong>${escapeHtml(categoryTitle)}</strong></div></section>`;
