  ...
  return (
    <div style={{ background: "#edf1f3", padding: 20, borderRadius: 10, overflow: "auto" }}>
      <div style={{ width: 720, minHeight: 980, background: "white", margin: "0 auto", padding: "30px 42px", boxShadow: "0 8px 25px #ccd5da", fontFamily: "Arial", fontSize: 12 }}>
        ...
        <table style={{ width: "100%", borderCollapse: "collapse" }}><thead><tr><th style={{ border: "1px solid #444", padding: 7, width: 65 }}>Quantità</th><th style={{ border: "1px solid #444", padding: 7, textAlign: "left" }}>Dettaglio fornitura</th><th style={{ border: "1px solid #444", padding: 7, width: 125 }}>Prezzo IVA inclusa</th></tr></thead><tbody>
          {(data.items || []).map((item: any, index: number) => {
            const meta = itemMeta(item);
            const structural = ["TITOLO", "TESTO"].includes(item.type);
            return <tr key={index}><td style={{ border: "1px solid #555", padding: 8, verticalAlign: "top", textAlign: "center" }}>{structural ? "" : item.quantity}</td><td style={{ border: "1px solid #555", padding: 8, whiteSpace: "pre-line", fontWeight: item.type === "TITOLO" ? 800 : 400 }}><div style={{ display: "flex", gap: 10 }}>{item.productId && <img src={`/api/products/${item.productId}/image`} alt="" style={{ width: 58, height: 58, objectFit: "contain" }} />}<div>{item.title && <strong>{item.title}<br /></strong>}{[meta.brand, meta.model].filter(Boolean).length > 0 && <small>Marca e modello: {[meta.brand, meta.model].filter(Boolean).join(" · ")}<br /></small>}{item.description || item.title}</div></div></td><td style={{ border: "1px solid #555", padding: 8, textAlign: "right", verticalAlign: "top" }}>{structural ? "" : formatCurrency(Math.round((Number(String(item.unitPrice || "0").replace(",", ".")) || 0) * 100 * (Number(item.quantity) || 0)))}</td></tr>;
          })}
        </tbody></table>
        <div style={{ width: 330, marginLeft: "auto" }}><div style={{ display: "flex", justifyContent: "space-between", border: "1px solid #555", padding: 8 }}><strong>Prezzo totale</strong><strong>{formatCurrency(totals.totalCents)}</strong></div><div style={{ display: "flex", justifyContent: "space-between", border: "1px solid #555", borderTop: 0, padding: 8 }}><span>Incentivo richiesto</span><strong>{formatCurrency(incentive.incentiveAmountCents)}</strong></div><div style={{ display: "flex", justifyContent: "space-between", border: "1px solid #555", borderTop: 0, padding: 8 }}><span>Totale al netto dell’incentivo<br /><small>IVA inclusa</small></span><strong>{formatCurrency(incentive.netAfterIncentiveCents)}</strong></div></div>
