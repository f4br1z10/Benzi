...
export async function DELETE(_: NextRequest, { params }: Context) { try { const id = Number((await params).id); const itemCount = await prisma.quoteItem.count({ where: { productId: id } }); if (itemCount) { const product = await prisma.product.update({ where: { id }, data: { active: false } }); return NextResponse.json({ archived: true, product }); } const product = await prisma.product.findUnique({ where: { id }, include: { attachments: true } }); await prisma.product.delete({ where: { id } }); await Promise.all(product?.attachments.map((a) => fs.unlink(a.path).catch(() => undefined)) || []); return NextResponse.json({ deleted: true }); } catch (error) { return handleApiError(error, "eliminazione prodotto"); } }
